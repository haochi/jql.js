import { Execution, ExecuteResult, TableAs, TableRowReference, Predicate } from './Query'
import { JoinType } from './JoinType'
import { Row } from '../Table'

type JoinStrategy<R> = (intermediateResult: ExecuteResult, otherTable: TableAs<R>, predicate: Predicate) => ExecuteResult
...
export class JoinExecution<T, R> implements Execution {
    ...
    private static innerJoin<R>(intermediateResult: ExecuteResult, otherTable: TableAs<R>, predicate: Predicate): ExecuteResult {
    ...
    private static leftJoin<R>(intermediateResult: ExecuteResult, otherTable: TableAs<R>, predicate: Predicate): ExecuteResult {
        const result: ExecuteResult = []
        intermediateResult.forEach(tableRow => {
            let joined = false;
    ...
    private static rightJoin<R>(intermediateResult: ExecuteResult, otherTable: TableAs<R>, predicate: Predicate): ExecuteResult {
        const result: ExecuteResult = [];
        otherTable.table.all().forEach(row => {
            let joined = false;
    ...
    private static fullJoin<R>(intermediateResult: ExecuteResult, otherTable: TableAs<R>, predicate: Predicate): ExecuteResult {
